...
| Scrapy 2.17.0 | Static HTTP acquisition, governed crawling, link discovery and robots handling; Python | Benchmarked and offline-reconstructed in isolated CPython 3.14 Windows x64 environment | BSD-3-Clause | 34 SHA-256-locked binary/pure-Python wheels; Twisted 26.4.0, cryptography 49.0.0, lxml 6.1.1 | Deterministic raw-byte capture, redirects, retry controls, Arabic/bilingual bytes and closed-origin policy | Primary benchmark architecture; production supervisor integration and broader hostile fixtures remain deferred |
| Playwright 1.61.1 / Chromium headless shell 149.0.7827.55 r1228 | JavaScript rendering, rendered DOM and browser network evidence; Node | Benchmarked and reconstructed from a pinned local Windows Chromium archive | Apache-2.0 package; Chromium/FFmpeg third-party notices retained in runtime | Node >=18; local browser, FFmpeg 1011 and Winldd 1007 | Deterministic rendered DOM, request interception, download detection, popup containment and measured five-process tree | Dynamic fallback only; Windows uses `--no-sandbox`; production supervisor integration and full browser licence inventory remain deferred |
