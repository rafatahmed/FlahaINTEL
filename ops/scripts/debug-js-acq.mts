...
import { PlaywrightAcquisitionAdapter, ScrapyAcquisitionAdapter } from "../../apps/api/src/acquisition/adapters.ts";
import { AcquisitionWorkflowService } from "../../apps/api/src/acquisition/service.ts";
import { SubmissionOrchestrator } from "../../apps/api/src/product/submission/orchestrator.ts";
...
console.log("before claim", job?.state, job?.selectedProviderId, job?.attemptCount, job?.maxAttempts, JSON.stringify(job?.selectionDecision)?.slice(0, 300));
...
console.log(
  "after",
  job?.state,
  job?.attempts.map((a) => ({ state: a.state, errorCode: a.errorCode, msg: a.errorMessage?.slice(0, 400) })),
);
console.log(
  "transitions",
